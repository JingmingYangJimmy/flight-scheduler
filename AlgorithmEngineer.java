...
public class AlgorithmEngineer<NodeType, Distance extends Number> implements GraphADT<NodeType,
Distance>, IAlgorithmEngineer<NodeType, Distance> {
  ...
  protected class Edge {
    ...
    public Edge(Vertex target, Distance weight) {
      this.target = target;
      this.weight = weight;
    }
  }
  ...
  public AlgorithmEngineer() { vertices = new Hashtable<>(); }
   ...
   */
  public boolean insertEdge(NodeType source, NodeType target, Distance weight) {
   ...
   */
  public Distance getWeight(NodeType source, NodeType target) {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      // copy starting vertex
      this(copyPath.start);
      // copy distance and add new distance
      this.distance = copyPath.distance + extendBy.weight.doubleValue();
      // deep copy data sequence
      this.dataSequence = new LinkedList<>();
      for(NodeType v : copyPath.dataSequence){
        this.dataSequence.add(v);
      }
      // add new target data to data sequence
      this.dataSequence.add(extendBy.target.data);
      // update ending vertex
  ...
  protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    ...
    if(start == null || end == null) throw new NoSuchElementException("Start or End can't be null");
    if(!vertices.containsKey(start)) throw new NoSuchElementException("Graph does not contain " +
        "starting vertex");

    // instantiate new priority queue for discovered paths
    PriorityQueue<Path> frontier = new PriorityQueue<>();
    // create hashtable for visited vertices and their paths
    Hashtable<NodeType, Path> visited = new Hashtable<>();

    // get starting vertex
    Vertex startVertex = vertices.get(start);

    // create new path with 0 distance
    frontier.add(new Path(startVertex));

    while(!frontier.isEmpty()){
      // get and remove head of list
      Path currPath = frontier.poll();
      // get current vertex, so you can loop through all of its edges leaving
      Vertex currVertex = currPath.end;
      // add vertex and path to visited
      visited.put(currVertex.data, currPath);

      // loop through each leaving edge from currentVertex
      for(Edge edge : currVertex.edgesLeaving){

        // create var for edge target data
        NodeType targetData = edge.target.data;

        // check to see if new edge already has been visited
        if(!visited.containsKey(targetData)) {

          boolean pathExists = false;
          for (Path inFrontierPath : frontier) {
            if (inFrontierPath.end.data == targetData) {
              // if a path already exists, create new path to compare
              pathExists = true;
              Path newPath = new Path(currPath, edge);
              // compare
              if (inFrontierPath.compareTo(newPath) > 0) {
                // if path in frontier is greater than new path then remove p and add new Path
                frontier.remove(inFrontierPath);
                frontier.add(newPath);
                break;
              }
            }
          }
          // if the path does not exist add new path
          if(!pathExists) {
            frontier.add(new Path(currPath, edge));
          }
        }
      }
    }
    Path shortestPath = visited.get(end);
    // throw NoSuchElementException if path is null
  ...
  /**
   * This function returns a List of all airports in the graph
   *
   * @return a list of all airports in the graph
   */
  public List<NodeType> returnAllAirports() {
    List<NodeType> airportList = new ArrayList<>();
    Set<NodeType> airportSet =  vertices.keySet();
    for(NodeType key : airportSet){
      airportList.add(key);
    }
