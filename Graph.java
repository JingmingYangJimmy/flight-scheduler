...
// TA: <Karan>
// Lecturer: <Gary Dahl>
// Notes to Grader: <I am data wrangler>
...
public class Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
  ...
  protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    ...
    if(containsVertex(start)==false)
      throw new NoSuchElementException("no start");
    if(containsVertex(end)==false)
      throw new NoSuchElementException("no end");
    if(vertices.get(start).edgesLeaving==null)
      throw new NoSuchElementException("no path");
    if((vertices.get(start).edgesLeaving.size()==0))
      throw new NoSuchElementException("no path");

    Vertex origin=vertices.get(start);
    Vertex ending=vertices.get(end);
    Vertex current=origin;
    PriorityQueue <Path>queue=new PriorityQueue<>(); // store all of the possible paths


    Path path=new Path(origin); //The beginning path
    Path result=null; //The path that has the minimum costs

    ArrayList <Vertex>list=new ArrayList<>(); //storing the vertices, to check if the next vertex
    //is been through or not
    list.add(origin);


    for(int i=0;i<current.edgesLeaving.size();i++) { //we add the edges first
      queue.add(new Path(path,current.edgesLeaving.get(i)));
    }


    while(queue.size()>0&&current!=ending) { //if the queue is empty or we encounter the end, 
      //we will exit the while loop
      path=queue.peek(); //we use the most cheapest path to start
      current=queue.remove().end; //we remove that path from the queue, and we update the current
      //vertex


      if(list.contains(current)) //if we have already go 
        //though the vertex, we will see other possible combinations
        continue;

      list.add(current); //if we do not have that node, we will add it now

      if(current==ending) { //if we find what we want, we will exit the loop, since we are always
        //using the minimum costs to find.
        result=path;
        break;
      }


      for(int x=0;x<current.edgesLeaving.size();x++) {
        queue.add(new Path(path,current.edgesLeaving.get(x)));
      }
    }

    if(result==null)//if we do not find the ending, we will throw an exception
      throw new NoSuchElementException ("no path");
